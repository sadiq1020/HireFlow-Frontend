...
import { CheckCircle, Loader2, Mail, Phone, Save, User } from 'lucide-react';
...
export default function SeekerProfilePage() {
  ...
  // Track the avatar URL locally so we can save it with the form
  ...
  const { mutate: updateProfile, isPending } = useMutation({
    mutationFn: (payload: ProfileForm & { image?: string }) =>
  ...
  const displayImage = pendingImageUrl ?? profile?.image;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      >
        <h3 className="text-sm font-bold text-foreground mb-5">Profile Photo</h3>
        <ImageUploader
          ...
          label="Your profile photo is shown on your applications"
        />

        {/* Name + badges below uploader */}
