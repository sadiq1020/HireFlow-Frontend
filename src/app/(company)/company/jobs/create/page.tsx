...
import { AIJobDescriptionPanel } from '@/components/ai/AIJobDescriptionPanel';
...
export default function CreateJobPage() {
  ...
  // Called when the AI panel's "Apply to Form" button is clicked
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4 group cursor-pointer"
        >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          <button
            type="button"
            onClick={() => router.back()}
            className="px-6 py-3 bg-secondary hover:bg-secondary/80 text-foreground font-semibold rounded-xl text-sm transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={isPending}
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-primary hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-semibold rounded-xl transition-all hover:shadow-lg hover:shadow-primary/25 text-sm cursor-pointer"
          >
            {isPending ? (
              <>
