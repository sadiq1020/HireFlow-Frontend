...
export default function JobDetailsClient({ initialJob: job }: { initialJob: IJob }) {
  ...
  const hasApplied = applicationsData?.data?.some((app: any) => app.jobId === job.id);
  const isLoading = appsLoading && !!session?.user;
  ...
  const isAlreadySaved = (savedJobsData as any)?.data?.some(
    (s: any) => s.jobId === job.id || s.job?.id === job.id
  ) ?? false;

  // ── Save mutation (optimistic) ──────────────────────────────────────────────
  const { mutate: saveJob, isPending: isSaving } = useMutation({
    mutationFn: () => api.post('/saved-jobs', { jobId: job.id }),
    onMutate: async () => {
      ...
      queryClient.setQueryData(['saved-jobs'], (old: any) => {
        ...
        return { ...old, data: [...old.data, { id: `optimistic-${job.id}`, jobId: job.id, job }] };
      });
      return { previous };
    },
  ...
  const { mutate: unsaveJob, isPending: isUnsaving } = useMutation({
    mutationFn: () => api.delete(`/saved-jobs/${job.id}`),
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ['saved-jobs'] });
      const previous = queryClient.getQueryData(['saved-jobs']);
      queryClient.setQueryData(['saved-jobs'], (old: any) => {
        ...
        return {
          ...old,
          data: old.data.filter((s: any) => s.jobId !== job.id && s.job?.id !== job.id),
        };
      });
      return { previous };
    },
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="flex items-start gap-5 mb-6">
                ...
                <span className={`px-3 py-1.5 rounded-full text-xs font-bold border flex-shrink-0 ${jobTypeColors[job.type] || jobTypeColors.FULL_TIME}`}>
                  {jobTypeLabels[job.type] || jobTypeLabels.FULL_TIME}
                </span>
              </div>
                ...
                <button
                  ...
                  className={`flex-1 py-3 font-semibold rounded-xl transition-all text-sm ${
                    hasApplied
                      ...
                      : 'bg-primary hover:bg-primary/90 text-primary-foreground hover:shadow-lg hover:shadow-primary/25 disabled:opacity-50'
                  }`}
                >
                  {hasApplied ? (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold border ${jobTypeColors[job.type] || jobTypeColors.FULL_TIME}`}>
                    {jobTypeLabels[job.type] || jobTypeLabels.FULL_TIME}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
